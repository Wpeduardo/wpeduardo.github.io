...
classes: wide
header:
  teaser: /assets/images/htb-writeup-querier/image005.png
categories:
  - hackthebox
  - infosec
tags:  
...
Luego pasaremos a la fase de Escaneo y Enumeración con el fin de poder escanear los puertos del nodo. Además, obtendremos los servicios que se han levantado en los puertos abiertos, las versiones de los servicios, y el sistema operativo de la máquina EasyCTF.
-Para esta fase utilizaremos `Nmap`.Donde le pasaremos los siguientes parametros:
...
Además, utilizare el script http-enum.nse, que también realiza un fuerza bruta de directorios con una lista de nombres posibles de directorios y archivos con el fin de encontrar archivos y directorios escondidos.
![](/assets/images/EasyCTF/image031.png)

Llegamos a obtener un archivo escondido, que está almacenado en el directorio raíz del servidor web.
Ahora, accederemos al archivo robots.txt a través del navegador web.
![](/assets/images/EasyCTF/image033.png)

No llegamos a encontrar información relevante en este archivo.
...
Donde llegamos a observar que el recurso obtenido anteriormente, viene a ser la página de inicio predeterminada del Sistema de Gestión de Contenido `CMS Made simple`, que permite crear y administrar aplicaciones web sin tener conocimientos avanzados en programación web.

Además, llegamos a obtener la versión del CMS que se tiene almacenada en el servidor web de la máquina EasyCTF.

Además, en la página de inicio del CMS.Llegamos a encontrar la palabra here, que nos dirige a otro recurso.
...
Para verificar esta hipótesis, utilizamos `Burp Suite` para capturar la solicitud POST HTML que hacemos al servidor web, para que cargue nuestro archivo PHP. Luego enviaremos la solicitud capturada a la ventana Repeater para poder observar la respuesta HTML que recibimos del servidor web.
